return function(Window, Tabs, ESP)
    ...
    TeleportGroup:AddButton({ Text = 'Teleport Back', Func = function() workspace:WaitForChild("Network"):WaitForChild("Teleport-RemoteEvent"):FireServer("Aquarium") end })
                ...
                if zone then
                    ...
                    if HardcodedZones[zone] then
                        ...
                        if char then
                            ...
                            Library:Notify("Auto-teleported to " .. zone)
                        end
                    end
                end
            end
    ...
    local function getFishData(fish)
        ...
        if bp then
            ...
            if frame then
                ...
                if mLab then mut = mLab.Text:gsub("<[^>]+>", ""):lower() end
                local rLab = frame:FindFirstChild("Rarity")
                if rLab then rar = rLab.Text:gsub("<[^>]+>", ""):lower() end
            end
        end
        return mut ~= "" and mut or "none", rar ~= "" and rar or "normal"
    end
    ...
    local function checkFilters(mut, rar, mFilters, rFilters)
        ...
        local mCount = 0 for _ in pairs(mFilters) do mCount = mCount + 1 end
        ...
        local rCount = 0 for _ in pairs(rFilters) do rCount = rCount + 1 end
            ...
            if autoFarmEnabled then
                for _,v in pairs(Fish:GetChildren()) do
                    ...
                    if v:IsA("Model") then
                        ...
                        elseif targetFishInput ~= "" and string.find(v.Name:lower(), targetFishInput:lower()) then match = true end
                    ...
                    if f:IsA("Model") then
                        ...
                        if checkFilters(mut, rar, espMFilters, espRFilters) then
                            if not ESP.Objects[f] then
                                ...
                                ESP:Add(f, {
                                    Name = f.Name .. (mut ~= "none" and " ["..mut.."]" or "") .. (rar ~= "normal" and " ["..rar.."]" or ""),
                                    PrimaryPart = f:FindFirstChild("RootPart"),
                                    Color = color,
                                    IsEnabled = function() return fishEspEnabled and f.Parent == Fish and checkFilters(mut, rar, espMFilters, espRFilters) end
                                })
                            end
                        else
                            if ESP.Objects[f] then ESP:Remove(f) end
                        end
                    end
                end
            end
            ...
            if autoBuyTreats or autoBuyTools then
                ...
                if pUI and pUI:FindFirstChild("Shops") then
                    if autoBuyTreats and pUI.Shops:FindFirstChild("Treat") then
                        ...
                        if content then for _, item in pairs(content:GetChildren()) do local slot = item:FindFirstChild("SlotTemplate") local stock = slot and tonumber(string.match(slot.StockAmount.Text, "%d+")) or 0 if stock > 0 then for i=1, stock do if not autoBuyTreats then break end fireBuy("Treat", item.Name) task.wait(0.1) end end end end
                    end
                    if autoBuyTools and pUI.Shops:FindFirstChild("Tool") then
                        ...
                        if content then for _, item in pairs(content:GetChildren()) do local slot = item:FindFirstChild("SlotTemplate") local stock = slot and tonumber(string.match(slot.StockAmount.Text, "%d+")) or 0 if stock > 0 then for i=1, stock do if not autoBuyTools then break end fireBuy("Tool", item.Name) task.wait(0.1) end end end end
                    end
                end
            end
            task.wait(3)
    ...
    ModGroup:AddSlider('SwimSpeed', { Text = 'Swim Speed', Min = 1, Max = 20, Default = 1, Rounding = 0, Callback = function(v) workspace:SetAttribute("AdminSpeedMultiplier", v) if player.Character and player.Character:FindFirstChild("Humanoid") then player.Character.Humanoid.WalkSpeed = 16 * v end end })
    local reachEnabled = false
    ModGroup:AddToggle('Reach', { Text = 'Reach / Instant Interact', Default = false, Callback = function(v) 
        reachEnabled = v 
        if v then
            for _, p in pairs(workspace:GetDescendants()) do if p:IsA("ProximityPrompt") then p.HoldDuration = 0 p.MaxActivationDistance = 50 p.RequiresLineOfSight = false end end
        end
    end })
    ...
    task.spawn(function() while true do if autoEquipBest then pcall(equipBest) end task.wait(5) end end)
            ...
            if smartSellEnabled then
                for _, tool in pairs(player.Backpack:GetChildren()) do
                    if tool:IsA("Tool") then
                        local rarity = "Normal"
                        for _, c in pairs(tool:GetChildren()) do if c:IsA("BasePart") then local bgui = c:FindFirstChild("BillboardGui") if bgui and bgui:FindFirstChild("Frame") and bgui.Frame:FindFirstChild("Rarity") then rarity = bgui.Frame.Rarity.Text:gsub("<[^>]+>", "") break end end end
                        if rarity == "Normal" then rarity = tool.Name:match("%[(.-)%]") or "Normal" end
                        
                        local shouldSell = false
                        local count = 0 for _ in pairs(smartSellFilters) do count = count + 1 end
                        if count == 0 then shouldSell = true else for k,v in pairs(smartSellFilters) do if v and string.find(rarity:lower(), k:lower()) then shouldSell = true break end end end
